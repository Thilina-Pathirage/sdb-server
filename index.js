...
const db=mysql.createConnection({host:"sdb33-server.mysql.database.azure.com", user:"vibckskeec", password:"12O1L54BRI8L32X6$", database:"sdb33-database", port:3306});
...
app.get('/search', authenticateUser, async (req, res) => {
  ...
  try {
    ...
    if (searchTerm === '*') {
      ...
      // Fetch customers based on search term and user role with parameterized query
      query = 'SELECT * FROM customers WHERE name LIKE ? OR email LIKE ?';
    }

    // Using the ? as placeholders and passing the search term twice to prevent SQL injection
